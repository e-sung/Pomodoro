...
open class RippleButton: UIButton {
    ...
    fileprivate func setupRippleView() {
        ...
        let originX: CGFloat = (bounds.width / 2) - (size / 2)
        let originY: CGFloat = (bounds.height / 2) - (size / 2)
        let corner: CGFloat = size / 2

        rippleView.backgroundColor = rippleColor
        rippleView.frame = CGRect(x: originX, y: originY, width: size, height: size)
        rippleView.layer.cornerRadius = corner
    }

    open override func beginTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        touchCenterLocation = trackTouchLocation ? touch.location(in: self) : nil
